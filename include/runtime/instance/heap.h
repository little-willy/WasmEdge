...
public:
  HeapInstance(const AST::ArrayType &Type, uint32_t Size)
      : Type(Type),
        Payload(Size * Type.getFieldType().getStorageType().size()) {}
  HeapInstance(const AST::ArrayType &Type, uint32_t Size,
               Span<const Byte> InitPayload)
      : Type(Type), Payload(InitPayload.begin(), InitPayload.end()) {
    assuming(Size * Type.getFieldType().getStorageType().size() ==
             InitPayload.size());
  }
  HeapInstance(const AST::StructType &Type)
      : Type(Type), Payload(Type.size()) {}
  HeapInstance(uint32_t Value) : Type(Value) {}
